//! Build-time codegen for the language table.
//!
//! Reads `languages.json5` (originally authored by codestats, MIT) and emits
//! `languages.rs` into `OUT_DIR`, which is `include!`d by `src/langs/data.rs`.
//! Generating at build time keeps the dataset (460+ languages) out of the
//! compile-time macro surface while still producing a fully static `&[Language]`.
    ...
    D: Deserializer<'de>,
{
    deserialize_vec_strings(deserializer, "line_comments", true, |s, idx| {
        if s.is_empty() {
            Err(format!("line comment {}: cannot be empty", idx + 1))
        } else {
            Ok(())
        }
    })
...
fn deserialize_block_comments<'de, D>(
    deserializer: D,
) -> result::Result<Vec<(String, String)>, D::Error>
where
    D: Deserializer<'de>,
{
...
fn normalize_languages(entries: IndexMap<String, LanguageConfig>) -> Result<Vec<LanguageConfig>> {
    ...
    for (index, (name, mut config)) in entries.into_iter().enumerate() {
        if name.trim().is_empty() {
            errors.push(format!(
                "Language at position {}: name cannot be empty",
                index + 1
            ));
            continue;
        }
...
fn validate_languages(languages: &[LanguageConfig]) -> Result<()> {
    ...
    let mut seen_patterns: HashMap<String, PatternInfo> = HashMap::new();
    for lang in languages {
        if !seen_names.insert(lang.name.clone()) {
            errors.push(format!("Duplicate language name '{}'", lang.name));
        }
        if lang.name.trim() != lang.name {
            errors.push(format!(
                "Language '{}': name has leading/trailing whitespace",
                lang.name
            ));
        }
        for pattern in &lang.file_patterns {
            let info = seen_patterns.entry(pattern.clone()).or_insert(PatternInfo {
                names: Vec::new(),
    ...
    for (pattern, info) in &seen_patterns {
