...
pub(super) static LANGUAGE_GLOBSET: LazyLock<LanguageGlobs> = LazyLock::new(|| {
    ...
    for lang in LANGUAGES {
        for pattern in lang.file_patterns {
            ...
            let glob = glob_builder.build().unwrap_or_else(|e| {
                panic!(
                    "Invalid glob pattern '{pattern}' for language {}: {e}",
                    lang.name
                )
            });
    ...
    let set = builder
        .build()
        .expect("GlobSetBuilder should never fail with valid patterns from languages.json5");
    LanguageGlobs {
        set,
        pattern_lang_indexes,
    }
});
